 ...
 * Description:
 */
public class HomeClassifyChildAdapter extends BaseQuickAdapter<CourseContentBean, BaseViewHolder> {
    public HomeClassifyChildAdapter(int layoutResId, @Nullable List<CourseContentBean> data) {
        super(layoutResId, data);
    }

    @Override
    protected void convert(BaseViewHolder helper, CourseContentBean item) {
        ...
        helper.setText(R.id.tv_course_price, "¥" + item.getCurrentPrice() + "/年");
            ...
            if (item.isVipFlag()) {
                ...
                if (item.isBuy()) {
                    ...
                    if (item.isFree()) {
                        ...
                        if (item.getOriginalPrice().equals(item.getCurrentPrice())) {
                            //价格相同 原价不展示
                            helper.setGone(R.id.tv_course_original_price, false);
                        } else {
                            helper.setGone(R.id.tv_course_original_price, true);
